...
import { TimetableEntry, Subject, UserProfile } from '../types';
...
const formatTimeTo12Hr = (time24: string): string => {
  if (!time24) return '';
...
const calculateEndTime = (startHourStr: string): string => {
  if (!startHourStr) return '';
  const [h, m] = startHourStr.split(':');
  const nextHour = (parseInt(h, 10) + 1).toString().padStart(2, '0');
  return `${nextHour}:${m}`;
};

const Timetable: React.FC<TimetableProps> = ({ timetable, setTimetable, subjects, profile, setProfile }) => {
  ...
  const activeDays = profile?.isSaturdayWorking
  ...
  const handleAddEntry = (e: React.FormEvent) => {
    ...
    const start = newEntry.startTime;
    const end = newEntry.endTime;
    if (!start || !end) return alert("Please select start and end times.");

    if (start < "09:00" || end > "17:00" || start > "17:00") {
      return alert("Classes can only be scheduled between 9:00 AM and 5:00 PM.");
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)] animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        ...
        {profile?.isSaturdayWorking !== undefined && (
          <div className="flex items-center gap-3">
            <button
              onClick={() => setProfile(prev => prev ? { ...prev, isSaturdayWorking: !prev.isSaturdayWorking } : null)}
              className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-950 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center gap-2 active:scale-95 shadow-sm"
            >
              <i className={`fa-solid ${profile.isSaturdayWorking ? 'fa-toggle-on text-indigo-500' : 'fa-toggle-off text-slate-400'} text-base`}></i>
      ...
      {profile?.isSaturdayWorking === undefined ? (
        <div className="flex-1 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-center p-6 min-h-[400px]">
          <div className="max-w-md w-full text-center space-y-6 animate-in zoom-in-95 duration-300">
            <div className="w-16 h-16 bg-indigo-50/50 dark:bg-indigo-900/30 rounded-2xl flex items-center justify-center mx-auto text-indigo-600 dark:text-indigo-400 text-2xl">
              <i className="fa-solid fa-calendar-week"></i>
            </div>
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-slate-800 dark:text-white">Configure Your Timetable</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Does your institute schedule classes on Saturdays? Select your weekly structure to customize your timetable view.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4 pt-2">
              <button
                type="button"
                onClick={() => setProfile(prev => prev ? { ...prev, isSaturdayWorking: false } : null)}
                className="p-5 bg-slate-50 dark:bg-slate-800/50 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 border border-slate-200 dark:border-slate-800 hover:border-indigo-300 rounded-2xl text-left space-y-2 transition-all group active:scale-95 flex flex-col justify-between h-36"
              >
                <div>
                  <div className="text-[10px] font-black text-slate-400 group-hover:text-indigo-500 transition-colors uppercase tracking-wider">5-Day Week</div>
                  <div className="text-sm font-bold text-slate-700 dark:text-slate-200 mt-1">Monday - Friday</div>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-normal">Saturday & Sunday are weekends.</p>
              </button>
              
              <button
                type="button"
                onClick={() => setProfile(prev => prev ? { ...prev, isSaturdayWorking: true } : null)}
                className="p-5 bg-slate-50 dark:bg-slate-800/50 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 border border-slate-200 dark:border-slate-800 hover:border-indigo-300 rounded-2xl text-left space-y-2 transition-all group active:scale-95 flex flex-col justify-between h-36"
              >
                <div>
                  <div className="text-[10px] font-black text-slate-400 group-hover:text-indigo-500 transition-colors uppercase tracking-wider">6-Day Week</div>
                  <div className="text-sm font-bold text-slate-700 dark:text-slate-200 mt-1">Monday - Saturday</div>
                </div>
                <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-normal">Saturday classes are included.</p>
              </button>
            </div>
          </div>
        </div>
              ...
              {hours.map((hour) => (
                <tr key={hour} className="group h-24">
                  <td className={`sticky left-0 z-20 p-2 border-b border-r border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/50 backdrop-blur-sm text-center transition-colors ${isNowInHour(hour) ? 'ring-2 ring-inset ring-indigo-500/20 bg-indigo-50/40 dark:bg-indigo-900/20' : ''}`}>
                    <span className={`text-[11px] font-black tabular-nums transition-colors ${isNowInHour(hour) ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-300 dark:text-slate-600'}`}>{formatTimeTo12Hr(hour)}</span>
                          ...
                          entries.map(cls => (
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between mt-1 text-[9px] font-black opacity-60">
                                <span>{formatTimeTo12Hr(cls.startTime)} - {formatTimeTo12Hr(cls.endTime)}</span>
                                <button 
            ...
            <form onSubmit={handleAddEntry} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="time" min="09:00" max="17:00" className="w-full bg-slate-50 dark:bg-slate-800 p-3 rounded-xl outline-none text-sm dark:text-white" value={newEntry.endTime} onChange={e => setNewEntry({...newEntry, endTime: e.target.value})} />
                </div>
              </div>
